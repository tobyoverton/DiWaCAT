...
void FindModes(vector<vector<double> >& WaveVec, vector<vector<double>>& WaveAmp, double b, double c, double mu, double epsilon, int nTheta, int nRadial, double kStep, bool PrintProgress) {
...
void ModeConvergence(vector<vector<double>>& ModeAmp, vector<vector<double>>& WaveVec, double rMax, double b, double c, double mu, double epsilon, double kStep, double ModeAccuracy) {
	int nR(10), nTheta(2);
	...
	while (ModeConverged == false) {
		...
		FindModes(WaveVectors, ModeAmplitude, b, c, mu, epsilon, nTheta, 1, kStep, false);
		double nTSum(0);
		for (int i = 1; i < nTheta; i++) {
			nTSum += abs(ModeAmplitude[i][0] * pow(rMax / b, i));
		...
		FindModes(WaveVec, ModeAmp, b, c, mu, epsilon, nTheta, nR, kStep, true);
		if (RModeConverged == true && TModeConverged == true) ModeConverged = true;
	}
}


void CalcWakeCyl(double &Fz, double &Fr, double &Ftheta, vector<vector<double>> RootAmp, vector<vector<double>> WaveVec, double r0, double r, double theta0, double theta, double b, double c, double z0, double z, double mu, double epsilon) {
	double zz = z - z0;
	if (zz < 0) {
		return;
	}
	...
	for (int j = 0; j < RootAmp.size(); j++) {
		//Loop through theta modes
		for (int i = 0; i < RootAmp[0].size(); i++) {
...
void TotalForceMeshCircular(vector<double> &Fz, vector<double> &Fr, vector<double> &Ftheta, vector<double> r, vector<double> theta, vector<double> zeta, vector<double> q, vector<vector<double>> ModeAmplitude, vector<vector<double>> WaveNums, double b, double c, double mu, double epsilon) {
	...
	for (int i = 0; i < TransverseMeshIndices.size(); i++) {
	...
	for (int i = 1; i < zValues.size(); i++) {
		...
		for (int j = 0; j < TransverseSize; j++) {
			...
			for (int k = 0; k < TransverseSize; k++) {
				...
				CalcWakeCyl(deltaFz, deltaFr, deltaFt, ModeAmplitude, WaveNums, rSource, rWitness, thetaSource, thetaWitness, b, c, 0, zDistance, mu, epsilon);
				vector<double> Calculations = { deltaFr, deltaFt, deltaFz };
